...
function JobMenuGarage(garageName)
    ...
    local vehicles = JobVehicles[job].vehicles[QBCore.Functions.GetPlayerData().job.grade]
...
function GangMenuGarage(garageName)
    local gang = QBCore.Functions.GetPlayerData().job.name
    local jobtype = QBCore.Functions.GetPlayerData().job.type
    local garage = Garages[garageName]

    if jobtype ~= 'illegal' then return end

    if not GangVehicles[gang] then
        return
    end
    local vehicleMenu = {
        {
            header = GangVehicles[gang].label,
            isMenuHeader = true
        }
    }

    local vehicles = GangVehicles[gang].vehicles[QBCore.Functions.GetPlayerData().job.grade]
    for veh, label in pairs(vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
    ...
    QBCore.Functions.TriggerCallback("qb-garage:server:GetGarageVehicles", function(result)
        if result == nil then
            ...
            for k, v in pairs(result) do
                ...
                local vname = 'Sõidukit ei eksisteeri'
                if vehData then
                    vname = vehData.brand..' '..vehData.name
                end
                if v.state == 0 then
                ...
                if type == "depot" then
                    ...
                    MenuGarageOptions[#MenuGarageOptions+1] = {
                        ...
                        description = Lang:t('menu.text.garage', {value2 = currentFuel, value3 = enginePercent, value4 = bodyPercent}),
                        event = 'qb-garages:client:TakeOutGarage',
                        args = {
                            vehicle = v,
                            vehicleModel = v.vehicle,
